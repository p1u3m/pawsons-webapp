...
The neutral foundation uses an opacity-driven depth model, while each personality house introduces its own accent palette. Rather than using a traditional gray scale, the system modulates `#111111` at varying opacities (0.03, 0.04, 0.4, 0.82–0.83) to create a unified tonal range. Every shade of gray on the page is technically the same hue — just more or less transparent. This creates a visual coherence that's nearly impossible to achieve with arbitrary hex values. The border system follows suit: `1px solid rgba(17, 17, 17, 0.16)` for light divisions and `1px solid rgba(17, 17, 17, 0.40)` for stronger interactive boundaries.

**Key Characteristics:**
- Warm paper background (`#F6F3EB`) as the global canvas
- Camera Plain Variable typeface with humanist warmth and editorial letter-spacing at display sizes
- Neutral opacity scale derived from `#111111`, paired with four distinct house accent palettes
- Inset shadow technique on buttons: `rgba(255,255,255,0.2) 0px 0.5px 0px 0px inset, rgba(0,0,0,0.2) 0px 0px 0px 0.5px inset`
- Neutral border palette: `rgba(17,17,17,0.16)` for subtle boundaries and `rgba(17,17,17,0.40)` for interactive elements
...
### Global / Neutral
- **Warm Paper** (`#F6F3EB`): Main page background. Soft cream / paper tone instead of pure white.
- **Ink** (`#111111`): Primary text, headings, dark CTA backgrounds.
- **White** (`#FFFFFF`): Button text and high-contrast light surfaces.
- **Muted Ink** (`rgba(17, 17, 17, 0.58)`): Secondary text, captions, descriptions.
- **Border** (`rgba(17, 17, 17, 0.16)`): Passive borders, separators, image outlines.

### Neutral Scale (Opacity-Based)
- **Ink 100%** (`#111111`): Primary text and dark surfaces.
- **Ink 83%** (`rgba(17,17,17,0.83)`): Strong secondary text.
- **Ink 82%** (`rgba(17,17,17,0.82)`): Body copy.
- **Ink 58%** (`rgba(17,17,17,0.58)`): Muted text and captions.
- **Ink 40%** (`rgba(17,17,17,0.40)`): Interactive borders and outlines.
- **Ink 16%** (`rgba(17,17,17,0.16)`): Passive borders and section dividers.
- **Ink 4%** (`rgba(17,17,17,0.04)`): Hover tint.
- **Ink 3%** (`rgba(17,17,17,0.03)`): Subtle overlay / surface depth.
...
- **Forget 100** (`#EEF6FF`)
- **Forget 300** (`#A9D7FF`)
- **Forget 500 / Primary** (`#61A9E6`)
- **Forget 600** (`#347EC0`)
- **Forget Cyan** (`#35C4DA`)
- **Forget 900** (`#183B64`)

#### Dandelion House
- **Dandelion 100** (`#FFF7D7`)
- **Dandelion 300** (`#FFE58D`)
- **Dandelion 500 / Primary** (`#F1C51B`)
- **Dandelion 600** (`#C8A311`)
- **Dandelion 700** (`#9B7F10`)
- **Dandelion 900** (`#493A0E`)

### Interactive
- **Primary CTA** (`#111111`): Main dark action button.
- **Primary CTA Text** (`#FFFFFF`)
- **Interactive Border** (`rgba(17,17,17,0.40)`)
- **Focus Ring** (`rgba(59,130,246,0.50)`): Keep a standard accessible blue focus indicator unless a house-specific interaction requires otherwise.
- **Focus Shadow** (`rgba(0,0,0,0.10) 0px 4px 12px`)

### House Usage Rule
Use the house colors as **section accents**, illustration support colors, labels, small decorative surfaces, and occasional CTA variants. Keep the global UI foundation neutral (`#F6F3EB` + `#111111`) so the four houses remain visually distinct without making the entire page overly saturated.
...
### Image Treatment
- Showcase/portfolio images with `1px solid rgba(17, 17, 17, 0.16)` border
...
- Suggestion pills with `rgba(17, 17, 17, 0.16)` borders
...
- Each card: image + title, `1px solid rgba(17, 17, 17, 0.16)` border, 12px radius
...
- **Section separation**: Footer uses `1px solid rgba(17, 17, 17, 0.16)` border and 16px radius container. Sections defined by generous spacing rather than border lines.
...
| Bordered (Level 1) | `1px solid rgba(17, 17, 17, 0.16)` | Cards, images, dividers |
...
### Do
- Use the warm warm paper background (`#F6F3EB`) as the page foundation — it's the brand's signature warmth
- Use Camera Plain Variable at display sizes with negative letter-spacing (-0.9px to -1.5px)
- Derive all grays from `#111111` at varying opacity levels for tonal unity
- Use the inset shadow technique on dark buttons for tactile depth
- Use `rgba(17, 17, 17, 0.16)` borders instead of shadows for card containment
...
- Don't use house accent colors everywhere — reserve them for their respective personality sections and decorative emphasis
...
- Don't mix border styles — `rgba(17, 17, 17, 0.16)` for passive, `rgba(17,17,17,0.40)` for interactive
...
### Image Behavior
- Template screenshots maintain `1px solid rgba(17, 17, 17, 0.16)` border at all sizes
...
- Heading text: Ink (`#111111`)
- Body text: Muted Ink (`rgba(17,17,17,0.58)`)
- Border: `rgba(17,17,17,0.16)` (passive), `rgba(17,17,17,0.40)` (interactive)
- Lavender House: `#9B73BB`
- Clover House: `#4D9C69`
- Forget-me-not House: `#61A9E6`
- Dandelion House: `#F1C51B`
- Focus: `rgba(0,0,0,0.10) 0px 4px 12px`
- Button text on dark: `#FFFFFF`

### Example Component Prompts
- "Create a hero section on warm paper background (#F6F3EB). Headline at 60px Camera Plain Variable weight 600, line-height 1.10, letter-spacing -1.5px, color #111111. Subtitle at 18px weight 400, line-height 1.38, color rgba(17, 17, 17, 0.58). Dark CTA button (#111111 bg, #FFFFFF text, 6px radius, 8px 16px padding, inset shadow) and ghost button (transparent bg, 1px solid rgba(17,17,17,0.40) border, 6px radius)."
- "Design a card on cream (#F6F3EB) background. Border: 1px solid rgba(17, 17, 17, 0.16). Radius 12px. No box-shadow. Title at 20px Camera Plain Variable weight 400, line-height 1.25, color #111111. Body at 14px weight 400, color rgba(17, 17, 17, 0.58)."
- "Build a template gallery: grid of cards with 12px radius, 1px solid rgba(17, 17, 17, 0.16) border, cream backgrounds. Each card: image with 12px top radius, title below. Hover: subtle border darkening."
- "Create navigation: sticky on cream (#F6F3EB). Camera Plain 16px weight 400 for links, #111111 text. Dark CTA button right-aligned with inset shadow. Mobile: hamburger menu with 6px radius."
- "Design a stats section: large numbers at 48px Camera Plain weight 600, letter-spacing -1.2px, #111111. Labels below at 16px weight 400, rgba(17, 17, 17, 0.58). Horizontal layout with 32px gap."

### Iteration Guide
1. Always use warm paper (`#F6F3EB`) as the global base — never pure white
2. Derive neutral grays from `#111111` at opacity levels; use house colors only as accents
3. Use `rgba(17,17,17,0.16)` borders for containment, not heavy shadows
